...
async def test_relay_defaults_when_no_config_entry(
    ...
    """Test relay uses default delay/state when config missing.

    When relay_configs has no entry for the relay letter, the entity
    should fall back to default hold delay and NO state interpretation.
    """
    from custom_components.akuvox.coordinator import AkuvoxCoordinatorData
    ...
    # Entity should still exist and show locked (NO: 0=locked)
    state = hass.states.get("lock.testlab_intercom_front_gate")
    assert state is not None
    assert state.state == "locked"
